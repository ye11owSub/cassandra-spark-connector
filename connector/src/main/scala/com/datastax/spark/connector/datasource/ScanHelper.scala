...
object ScanHelper extends Logging {
  ...
  def checkColumnsExistence(
      columns: Seq[ColumnRef],
      tableDef: TableDef): Seq[ColumnRef] = {
        ...
        case ColumnName(columnName, _) =>
          if (!allColumnNames.contains(columnName))
            throw new IOException(
              s"Column $column not found in table $keyspaceName.$tableName")
        case TTL(columnName, _) =>
          if (!regularColumnNames.contains(columnName))
            throw new IOException(
              s"TTL can be obtained only for regular columns, " +
  ...
  def tokenRangeToCqlQuery(
    ...
    val columns =
      cqlQueryParts.selectedColumnRefs.map(_.cql).mkString(", ")

    val hasUserTokenFilter: Boolean =
      cqlQueryParts.whereClause.predicates.exists(_.toUpperCase.contains("TOKEN("))

    val (cql, values) =
      if (containsPartitionKey(tableDef, cqlQueryParts.whereClause) || hasUserTokenFilter) {
        ("", Seq.empty)
      } else {
    ...
    val filter =
      (cql +: cqlQueryParts.whereClause.predicates)
        .filter(_.nonEmpty)
        .mkString(" AND ")
  ...
  def containsPartitionKey(
      tableDef: TableDef,
      clause: CqlWhereClause): Boolean = {
    val pk = tableDef.partitionKey.map(_.columnName).toSet
    val wherePredicates: Seq[Predicate] =
      clause.predicates.flatMap(CqlWhereParser.parse)
    val saiColumns =
      tableDef.indexes
        .filter(_.className.contains(StorageAttachedIndex))
        .map(_.target)
    ...
    val whereColumns: Set[String] = wherePredicates.collect {
      ...
      case RangePredicate(c, _, _)
          if pk.contains(c) && !saiColumns.contains(c) =>
        throw new UnsupportedOperationException(
          s"Range predicates on partition key columns (here: $c) are " +
            s"not supported in where. Use filter instead.")
    }.toSet

    val primaryKeyComplete =
      whereColumns.nonEmpty && whereColumns.size == pk.size
  ...
  def prepareScanStatement(
    ...
    try {
      ...
      case t: Throwable =>
        throw new IOException(
          s"Exception during preparation of $cql: ${t.getMessage}",
          t)
    }
  }
  ...
  def getPartitionGenerator(
    ...
    if (containsPartitionKey(tableDef, whereClause)) {
      ...
      partitionCount match {
        ...
        case None =>
          val estimateDataSize =
            new DataSizeEstimates(connector,
                                  tableDef.keyspaceName,
                                  tableDef.tableName).dataSizeInBytes
          val splitCount =
